...
void SaveToFile(Glib::ustring filename, Gtk::Window* parent_window){
    ...
    sprintf(temp,"harmonyseq file, generated by harmonySEQ %s. Manual editing is highly discouraged.",VERSION);
    kf.set_comment(temp);
    ...
    for (auto seq : seqs){
        ...
        if(auto noteseq = std::dynamic_pointer_cast<NoteSequencer>(seq)){
                kf.set_string(temp,"seq_type","note");
                ...
                kf.set_string(temp,"seq_type","ctrl");
...
bool LoadFileCurrent(Glib::KeyFile* kfp){
        ...
        for (int x = 0; x < seqNum; x++) {
            ...
            std::string seq_type = kfp->get_string(temp, "seq_type");
            if(seq_type == "note") seq = std::make_shared<NoteSequencer>();
            else if (seq_type == "ctrl") seq = std::make_shared<ControlSequencer>();
...
bool ConvertFile_0_16_to_0_17(Glib::KeyFile* kfp){
    ...
    for(int x = 0; x < seqnum; x++){
        ...
        /* While we're at it, fix ctrl types. */

        int seq_type = kfp->get_integer(temp,"seq_type");
        kfp->remove_key(temp, "seq_type");
        if(seq_type == 0) kfp->set_string(temp, "seq_type", "note");
        if(seq_type == 1) kfp->set_string(temp, "seq_type", "ctrl");
    }

    int evnum = kfp->get_integer( "System", "events_number");
    for (int e = 0; e < evnum; e++) {
        sprintf(temp, "Event %d", e);
        if(!kfp->has_group(temp)) continue;
        int actions_num = kfp->get_integer(temp, "actions");
        for (int a = 0; a < actions_num; a++) {
            sprintf(temp2, "Action_%d_type", a);
            if (!kfp->has_key(temp, temp2)) {
